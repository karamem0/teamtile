//
// Copyright (c) 2021-2026 karamem0
//
// This software is released under the MIT License.
//
// https://github.com/karamem0/teamtile/blob/main/LICENSE
//

import { defineMessages } from 'react-intl';
import parentMessages from '../messages';

const messages = {
  ...parentMessages,
  ...defineMessages({
    AllTeams: { defaultMessage: 'All teams' },
    ArchivedTeams: { defaultMessage: 'Archived teams' },
    Channels: { defaultMessage: 'Channels' },
    GitHub: { defaultMessage: 'GitHub' },
    GitHubLink: { defaultMessage: 'https://github.com/karamem0/teamtile' },
    Guests: { defaultMessage: 'Guests' },
    Members: { defaultMessage: 'Members' },
    NoChannelsFound: { defaultMessage: 'No channels found.' },
    NoMembersFound: { defaultMessage: 'No members found.' },
    NoTagsFound: { defaultMessage: 'No tags found.' },
    NoTeamsFound: { defaultMessage: 'No teams found.' },
    NoTeamsFoundDescription: { defaultMessage: 'It looks like you are not a member of any teams.' },
    NoTeamsFoundTitle: { defaultMessage: 'No teams found' },
    OpenChannelCalendar: { defaultMessage: 'Open channel calendar' },
    OpenInSharePoint: { defaultMessage: 'Open in SharePoint' },
    OpenLinkError: { defaultMessage: 'Cannot open the link.' },
    OperationNotSupported: { defaultMessage: 'This operation is not supported on this device.' },
    Owners: { defaultMessage: 'Owners' },
    Pin: { defaultMessage: 'Pin' },
    PinnedTeams: { defaultMessage: 'Pinned teams' },
    PrimaryChannel: { defaultMessage: 'Primary channel' },
    PrivacyPolicy: { defaultMessage: 'Privacy Policy' },
    PrivacyPolicyLink: { defaultMessage: 'https://github.com/karamem0/teamtile/blob/main/PRIVACY_POLICY.md' },
    Private: { defaultMessage: 'Private' },
    PrivateChannel: { defaultMessage: 'Private channel' },
    Public: { defaultMessage: 'Public' },
    Refresh: { defaultMessage: 'Refresh' },
    Reload: { defaultMessage: 'Reload' },
    SearchChannels: { defaultMessage: 'Search channels' },
    SearchMembers: { defaultMessage: 'Search members' },
    SearchTeams: { defaultMessage: 'Search teams' },
    SharedChannel: { defaultMessage: 'Shared channel' },
    Tags: { defaultMessage: 'Tags' },
    TermsOfUse: { defaultMessage: 'Terms of Use' },
    TermsOfUseLink: { defaultMessage: 'https://github.com/karamem0/teamtile/blob/main/TERMS_OF_USE.md' },
    Unpin: { defaultMessage: 'Unpin' },
    ViewChannels: { defaultMessage: 'View channels' },
    ViewMembers: { defaultMessage: 'View members' },
    ViewTags: { defaultMessage: 'View tags' }
  })
};

export default messages;
